...
#Winter 2026
#Copywrite 2026 Kurtis Marno Jones
...
BiocManager::install(pkgs =  c("marray","ctc",             #This first line tells R to go find the PACKAGES listed in the "". The c() 
                          "tidyverse","ape","cipheR"),     #command tells r that this is a list of multiple items; the "" around each 
...
library(ENTER_HERE)                                        #5 packages we just installed from Bioconductor (marray, ctc, tidyverse, ape, 
library(ENTER_HERE)                                        #and cipheR). Only enter one package for each of the library functions.
library(ENTER_HERE)
...
prenorm.array<-read.Spot(targets = Targets)               #Now that we have information on the experiment saved as Targets, we can read the
                                                           ...
                                                           #files to read. We'll save the data read from the files as a new VARIABLE 'prenorm.array'. 
                                                                                                                                  #***Hint HINT***
...
microarray.info$gnames@maInfo$Name[1631]<-.update.marray()
.gnames.update()

    prenorm.array@maLayout<-microarray.info$layout        #We overwrite the default maLayout and maGnames values with those we saved in 
    prenorm.array@maGnames<-microarray.info$gnames        #'microarray.info' to update this information.
...
normalized.array<-maNormMain(                              #The base marray normalization function is maNormMain and we will save this as a new
                              ...
                              VARIABLE_NAME,               #Next we need specify what is it that we want to normalize in this case it will be the
                                                           #data we just imported into R. Recall the VARIABLE that we saved the prenormalized 
                                                           #microarray data as.
                                                           
                                                           #Replace where is says VARIABLE_NAME with the VARIABLE for the prenormalized microarray data
  ...
  par(                                                     #To place multiple plots on one page, we need to change the parameters of how R generates
    ...
    plot(                                                  #To make a plot we will use the plot command,
         ...
         prenorm.array[,1],                                #then specify what data we want to plot (in this case, the first microarray experiment 
                                                           #given by 'prenorm.array[,1]' representing the Gene A Deletion Experiment)
...
kr<- kmeans(expression.data.filtered[2:5],5,nstart=25)

kc<- kmeans(t(expression.data.filtered[2:5]),2,nstart=25)


pdf(width=8.5, height = 11, file="FILE_NAME_HERE")        #Similar to when we used the pdf function before change where it says FILE_NAME_HERE 
